...
st.markdown("""
<style>
    h1 { text-align: center; color: #B22222; /* Rojo Ladrillo */ }
    .stButton > button { background-color: #B22222; color: white; border-radius: 5px; }
    .stDateInput > label, .stNumberInput > label, .stRadio > label { font-weight: bold; }
    .result-box { background-color: #f0f2f6; border: 1px solid #dcdcdc; border-radius: 5px; padding: 1em; margin-top: 1em; }
    .result-box h3 { color: #B22222; }
    .result-box strong { font-size: 1.2em; }
</style>
...
with col2:
    ...
    # Simplificación: asumimos convenio estándar, sin pluses complejos
...
with col_fin2:
    preaviso_incumplido_dias = st.number_input("Días de Preaviso Incumplido (si aplica)", min_value=0, step=1, value=0) # El empleador descuenta si no lo das
...
if st.button("Calcular Estimación"):
    if fecha_alta and fecha_baja and salario_bruto_anual >= 0:
        if fecha_baja < fecha_alta:
            ...
            meses_completos = antiguedad.months + (1 if antiguedad.days > 0 else 0) # Meses se cuentan completos
            antiguedad_total_meses = anios_completos * 12 + meses_completos
            antiguedad_anios_calculo = antiguedad_total_meses / 12.0 # Para el cálculo de indemnización

            # 2. Salario Diario (base para indemnización y finiquito)
            # Simplificación: Salario anual / 365. El cálculo real puede ser más complejo (últimas nóminas, etc.)
            salario_diario = salario_bruto_anual / 365.0

            # 3. Cálculo Indemnización
            indemnizacion = 0.0
            if tipo_despido == "Improcedente":
                # Simplificación: 33 días por año trabajado, prorrateando meses, con tope de 24 mensualidades (antes de 2012 era 45 días/tope 42)
                # Cálculo más preciso tendría en cuenta la fecha exacta de reforma laboral
                dias_indemnizacion = 33
                indemnizacion = salario_diario * dias_indemnizacion * antiguedad_anios_calculo
                # Aplicar tope (simplificado: 24 * salario mensual)
                ...
                # Simplificación: 20 días por año, prorrateando, tope 12 mensualidades
                dias_indemnizacion = 20
                indemnizacion = salario_diario * dias_indemnizacion * antiguedad_anios_calculo
                salario_mensual_aprox = salario_bruto_anual / 12.0
                tope_indemnizacion = 12 * salario_mensual_aprox
                indemnizacion = min(indemnizacion, tope_indemnizacion)
            # Si es Disciplinario, indemnizacion = 0

            # 4. Cálculo Finiquito
            # Salario días trabajados último mes (simplificación)
            dias_trabajados_ultimo_mes = fecha_baja.day
            salario_ultimo_mes = salario_diario * dias_trabajados_ultimo_mes

            # Parte proporcional pagas extra (si son 14 pagas)
            paga_extra_proporcional = 0.0
            if pagas_extra == "14":
                # Simplificación: Asume devengo semestral (Navidad/Verano)
                # Cálculo paga verano (devengo 1 Ene - 30 Jun)
                if fecha_baja.month <= 6:
                     dias_devengo_verano = (fecha_baja - date(fecha_baja.year, 1, 1)).days + 1
                     paga_extra_proporcional += (salario_bruto_anual / 14.0 / 180.0) * dias_devengo_verano # 180 días aprox por semestre
                else: # Devengada completa
                      paga_extra_proporcional += (salario_bruto_anual / 14.0)
                # Cálculo paga Navidad (devengo 1 Jul - 31 Dic)
                if fecha_baja.month > 6:
            ...
            # Descuento por preaviso incumplido (si aplica)
            descuento_preaviso = salario_diario * preaviso_incumplido_dias

            # Finiquito total (antes de impuestos/SS)
            ...
            st.markdown(f"""
             <div class="result-box" style="background-color: #e8f5e9;">
